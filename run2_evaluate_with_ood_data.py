...
dataloader = DataLoader(dataset, bs, shuffle=False, num_workers=4, pin_memory=True)
...
print(f"Loading descriptions: {hparams['descriptor_fname']}")
...
print("Evaluating...")
lang_accuracy_metric = torchmetrics.Accuracy(task='multiclass', num_classes=1000).to(device)
lang_accuracy_metric_top5 = torchmetrics.Accuracy(task='multiclass', num_classes=1000, top_k=5).to(device)

clip_accuracy_metric = torchmetrics.Accuracy(task='multiclass', num_classes=1000).to(device)
clip_accuracy_metric_top5 = torchmetrics.Accuracy(task='multiclass', num_classes=1000, top_k=5).to(device)


with open('/data2/youngju/visual_classification_with_LLM/data/imagenet-a/match_to_imagenet.json', 'r') as f:
    label_dct = json.load(f)

for batch_number, batch in enumerate(tqdm(dataloader)):
    images, labels = batch
    labels = [label_dct[f'{l}'] for l in labels]
